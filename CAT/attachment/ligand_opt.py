...
from .optimize_rotmat import optimize_rotmat, _get_angle
...
def allign_axis(mol: Molecule) -> None:
    ...
    ignore_idx = np.array(mol.properties.anchor_tup.anchor_group_idx, dtype=np.int64)
    xyz, idx_rot, idx_trans, idx_angle, angle = _get_allign_args(mol, mol.properties.anchor_tup)
    rotmat1 = optimize_rotmat(xyz, idx_rot, ignore_idx=ignore_idx)
...
def rdmol_from_array(rdmol: rdkit.Chem.Mol, array: "NDArray[np.float64]") -> None:
    """Convert an rdkit molecule into an array of Cartesian coordinates."""
    assert array.ndim == 2 and array.shape[1] == 3
    conf = rdmol.GetConformer(id=-1)
    for i, xyz in enumerate(array.tolist()):
...
def modified_minimum_scan_rdkit(ligand: Molecule, bond_tuple: Tuple[int, int]) -> None:
    ...
    * Finish with a final unconstrained geometry optimization.
    ...
    """
    ignore_idx = np.array(ligand.properties.anchor_tup.anchor_group_idx, dtype=np.int64)
    ...
    for rdmol in rdmol_list:
        ...
        rotmat = optimize_rotmat(xyz, idx_rot, ignore_idx=ignore_idx)
        ...
        if angle is not None:
            ...
            xyz = np.matmul(xyz, rotmat2.T, out=xyz)
        xyz -= xyz[idx_trans]
        rdmol_from_array(rdmol, xyz)

        # Compute the cost function
        cost = _get_angle(xyz, idx_rot, ignore_idx=ignore_idx)
